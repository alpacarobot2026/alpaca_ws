...
class RedundancyModule:
    ...
    def motion_model_redundancy(self, trk: Tracklet, input_data: FrameData, time_lag):
        ...
        for i, det in enumerate(candidate_dets):
            ...
            elif self.asso == 'center':
                dists.append(np.linalg.norm([pd_det.x - pred_bbox.x, pd_det.y - pred_bbox.y]))
        ...
        elif self.asso in ['m_dis', 'euler', 'center'] and (len(dists) == 0 or np.min(dists) > self.det_threshold):
            result_bbox = pred_bbox
            update_mode = 0 # two-stage not assiciated
        else:
            if self.asso in ['iou', 'giou']:
                result_bbox = candidate_dets[int(np.argmax(dists))]
            else:
                result_bbox = candidate_dets[int(np.argmin(dists))]
