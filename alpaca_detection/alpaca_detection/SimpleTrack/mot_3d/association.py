...
def bipartite_matcher(dets, tracks, asso, dist_threshold, trk_innovation_matrix):
    if asso == 'iou':
        ...
        dist_matrix = compute_center_distance(dets, tracks)
    elif asso == 'm_dis':
...
def greedy_matcher(dets, tracks, asso, dist_threshold, trk_innovation_matrix):
    ...
    if asso == 'center':
        distance_matrix = compute_center_distance(dets, tracks)
    elif asso == 'm_dis':
...
def compute_center_distance(dets, tracks):
    """Compute XY center distance, robust to bbox yaw/size jitter."""
    dist_matrix = np.empty((len(dets), len(tracks)))
    for i, det in enumerate(dets):
        for j, trk in enumerate(tracks):
            dist_matrix[i, j] = np.linalg.norm([det.x - trk.x, det.y - trk.y])
    return dist_matrix
