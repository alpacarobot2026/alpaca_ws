...
def nms_3d_dist_gpu(boxes, scores, l_ave=0.9, w_ave=0.5, nms_thresh=0.4):
    ...
    # Debug: log sizes and estimated device allocation to help diagnose OOM
    try:
        if boxes_bev.is_cuda:
            boxes_num = int(boxes_bev.shape[0])
            THREADS_PER_BLOCK_NMS = 8 * 8  # matches iou3d.cpp: sizeof(unsigned long long) * 8
            col_blocks = (boxes_num + THREADS_PER_BLOCK_NMS - 1) // THREADS_PER_BLOCK_NMS
            # mask allocation: boxes_num * col_blocks * sizeof(unsigned long long)
            mask_bytes = boxes_num * col_blocks * 8
            try:
                cuda_alloc = torch.cuda.memory_allocated(boxes_bev.device)
                cuda_reserved = torch.cuda.memory_reserved(boxes_bev.device)
            except Exception:
                cuda_alloc = None
                cuda_reserved = None

            # print(
            #     f"[DEBUG] nms_3d_dist_gpu: boxes.shape={tuple(boxes.shape)}, boxes_bev.shape={tuple(boxes_bev.shape)}, "
            #     f"boxes_num={boxes_num}, col_blocks={col_blocks}, expected_mask_bytes={mask_bytes} ({mask_bytes/1024**2:.2f} MB), "
            #     f"cuda_allocated={None if cuda_alloc is None else f'{cuda_alloc/1024**2:.2f} MB'}, "
            #     f"cuda_reserved={None if cuda_reserved is None else f'{cuda_reserved/1024**2:.2f} MB'}"
            # )
    except Exception as e:
        print("[DEBUG] nms_3d_dist_gpu logging failed:", e)
...
if __name__ == '__main__':
    ...
    paths = '/home/user/PycharmProjects/minkunet/bin/000000.txt'
