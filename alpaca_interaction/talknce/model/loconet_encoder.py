...
class locoencoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # pretrained=False: loconet.loadParameters() overwrites this whole
        # state dict from the TalkNCE checkpoint anyway, so the VGGish
        # torch.hub download here would just be discarded network traffic
        # (and a hard dependency on internet access at every cold start).
        self.audioEncoder = vggish.VGGish(
            urls={}, pretrained=False, preprocess=False, postprocess=False
        )
