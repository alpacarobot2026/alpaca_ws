    ...
    def loadParameters(self, path):
        ...
        if self.rank != None:
            ...
            for k, v in loadedState.items():
                k_clean = k.replace("model.module.", "")
                # Skip parameters whose shapes no longer match (e.g.,
                # audioEncoder conv1 after local architectural tweaks).
                if k_clean in selfState and selfState[k_clean].shape != v.shape:
                    continue
                new_state[k_clean] = v
