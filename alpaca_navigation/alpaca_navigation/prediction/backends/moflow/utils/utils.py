...
try:
    import git
except ImportError:  # pragma: no cover - optional dependency in the clean harness
    git = None
import logging
import shutil
import torch
...
except ImportError:  # pragma: no cover - only used for isinstance checks in vendor logging
    class EasyDict(dict):
        pass
...
def back_up_code_git(cfg, logger):
    if git is None:
        logger.info("GitPython not installed; skipping code backup metadata.")
        return
