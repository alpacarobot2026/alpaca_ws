...
from .mtr_encoder import SinusoidalPosEmb
...
class SocialTransformer(nn.Module):
    ...
    def forward(self, past_traj, mask):
        ...
        h_feat_ = self.transformer_encoder(h_feat, src_key_padding_mask=key_padding_mask)
...
class ETHEncoder(nn.Module):
    def __init__(self, config, use_pre_norm):
        ...
        num_input_context = int(getattr(self.model_cfg, "NUM_INPUT_CONTEXT", 6))
        past_frames = int(getattr(self.model_cfg, "PAST_FRAMES", 8))
        
        ### build social encoder
        self.agent_social_encoder = SocialTransformer(
            in_dim=num_input_context * past_frames,
            hidden_dim=256,
            out_dim=dim,
        )
    ...
    def forward(self, past_traj, agent_mask=None):
        """
        Args: [Batch size, Number of agents, Number of time frames, 6]
        """
        
        B, A, P, D = past_traj.shape
        agent_feature = self.agent_social_encoder(past_traj, mask=agent_mask)  # [B, A, D]
        ...
        encoder_out = self.transformer_encoder(agent_feature, src_key_padding_mask=key_padding_mask)
