...
class ETHMotionTransformer(nn.Module):
    ...
    def forward(self, y, time, x_data):
        ...
        expected_out_dim = int(self.model_cfg.MODEL_OUT_DIM)
        assert y.shape[-1] == expected_out_dim, f"Expected noisy future dim {expected_out_dim}, got {y.shape[-1]}"
        device = y.device
        B, K, A, _ = y.shape
        context_agent_mask = x_data.get('context_agent_mask', x_data.get('agent_mask'))
        forecast_agent_mask = x_data.get('forecast_agent_mask', context_agent_mask)

        ### context encoder
        encoder_out = self.context_encoder(
            x_data['past_traj_original_scale'],
            agent_mask=context_agent_mask,
        )  # [B, A, D]
        ...
        time_ = time
        if self.config.get('denoising_method', 'fm') == 'fm':
        ...
        agent_key_padding_mask = None
        if forecast_agent_mask is not None:
            agent_key_padding_mask = repeat(~forecast_agent_mask.bool(), 'b a -> (b k) a', k=K)
        y_emb_a = self.noisy_y_attn_a(y_emb_a, src_key_padding_mask=agent_key_padding_mask)
        ...
        readout_token = self.motion_decoder(query_token, t_emb, agent_mask=forecast_agent_mask)					# [B, K, A, D]	
...
class ETHIMLETransformer(nn.Module):
    ...
    def forward(self, x_data, num_to_gen=None):
        ...
        encoder_out = self.context_encoder(
            x_data['past_traj_original_scale'],
            agent_mask=x_data.get('agent_mask'),
        )  # [B, A, D]
